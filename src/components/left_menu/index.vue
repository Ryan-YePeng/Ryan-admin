  ...
  <el-scrollbar style="height:100%; background-color: #2f4055" v-show="!isSmall">
    ...
    >
      ...
      <template v-for="item in menuList">
        <!--一级菜单X-->
        <el-menu-item v-if="!item.children" :index="item.name" @click="jump($event, item.path)">
        ...
        <el-submenu v-if="item.children" :key="item.name" :index="item.name">
          ...
          <template v-for="citem in item.children">
            <!--二级菜单X-->
            <el-menu-item v-if="!citem.children" :index='citem.name' :key="citem.name"
                          @click="jump($event, citem.path)">
              <i :class="[citem.icon, 'iconfont']"></i>
              <span slot="title">{{citem.name}}</span>
            </el-menu-item>
            <!--二级菜单O-->
            <el-submenu v-if="citem.children" :key="citem.name" :index="citem.name">
              <template slot="title">
                <i :class="[citem.icon, 'iconfont']"></i>
                <span slot="title">{{citem.name}}</span>
              </template>
              <!--三级菜单X-->
              <el-menu-item v-if="!titem.children" v-for="titem in citem.children" :key="titem.name"
                            :index='titem.name' @click="jump($event, titem.path)">
                <i :class="[titem.icon, 'iconfont']"></i>
